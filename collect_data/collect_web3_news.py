...
class BianNewsSpider:
    def __init__(self):
        self.headers = {"clienttype": "web", "lang": "zh-cn"}
        self.headers_en = {
            "clienttype": "web",
        }
        ...
        for tag_id, info in id_info_mapping.items():
            ...
            if info.get("tag") in tags:
                ...
                Web3NewsTag.objects.create(
                    ...
                    url=f"https://www.binance.com/zh-CN/square/news/{quote(info.get('tag').lower())}",
                )
        ...
        news_lst = [
            {
                ...
                "webLink": _["webLink"],
                "authorName": _["title"],
                "published_time": datetime.fromtimestamp(_["date"]),
            }
            for _ in data
        ]
        pprint(news_lst)
